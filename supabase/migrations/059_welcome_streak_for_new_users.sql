-- ============================================================================
-- MIGRATION 059: Welcome Streak for New Users
-- ============================================================================
-- Purpose:
--   - Give new users a streak of 1 when they complete their first foundational task
--   - This creates immediate investment and increases return rate
--   - New users see streak=1 after naming their pet, creating urgency to return
--
-- Logic:
--   - When a foundational task is completed (e.g., name_pet)
--   - AND the user has streak=0 AND last_streak_date IS NULL (brand new user)
--   - THEN call increment_streak() to set their streak to 1
--
-- This means:
--   - Day 1: Complete onboarding → Streak = 1 (welcome reward)
--   - Day 2: Must do daily task → Streak = 2 (engagement loop)
-- ============================================================================
...
BEGIN
  ...
  -- 4. AUTO-SAVE RESILIENCE (Even if task exists) - Only for daily tasks
  ...
  -- ========================================================================
  -- 10. NEW: WELCOME STREAK for foundational tasks
  -- ========================================================================
  -- If this is a foundational task AND user is brand new (streak=0, no last_streak_date),
  -- award them their first streak as a welcome reward
  IF v_task_type = 'foundational' THEN
    SELECT COALESCE(streak, 0), last_streak_date 
    INTO v_user_streak, v_user_last_streak_date
    FROM public.profiles
    WHERE id = p_user_id;
    
    -- Brand new user: streak is 0 and they've never had a streak date
    v_is_new_user := (v_user_streak = 0 AND v_user_last_streak_date IS NULL);
    
    IF v_is_new_user THEN
      -- Award welcome streak (sets streak to 1)
      PERFORM public.increment_streak(p_user_id, p_timezone);
    END IF;
  END IF;
...
-- Add comment documenting the welcome streak logic
COMMENT ON FUNCTION award_task_points IS 'Awards points for completing pet tasks. For daily tasks, auto-awards secure_pet and increments streak. For foundational tasks completed by brand new users (streak=0, last_streak_date=NULL), awards a welcome streak of 1 to encourage return visits.';
